...
@pytest.fixture
def mock_wallet_db_data() -> Generator[MagicMock, None, None]:
...
@pytest.fixture
def mock_update_wallet_db_data() -> Generator[MagicMock, None, None]:
    mock_wallet = MagicMock()
    mock_wallet.id = 1
    mock_wallet.user_id = 123
...
async def test_get_wallet_found(mock_db: AsyncMock, mock_wallet_db_data: MagicMock):
    ...
    wallet_out = await get_wallet(
        user_id=mock_wallet_db_data.user_id,
        db=mock_db
    )
...
@pytest.mark.asyncio
async def test_add_into_wallet(mock_db: AsyncMock, mock_update_wallet_db_data: MagicMock):
    amount = 50
    mock_result = MagicMock()
    mock_db.execute.return_value = mock_result
    mock_result.scalar_one_or_none.return_value = mock_update_wallet_db_data
    wallet_out = await add_into_wallet(
        user_id=mock_update_wallet_db_data.user_id,
        amount=amount,
        db=mock_db
    )
